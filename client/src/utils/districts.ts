...
export const detectDistrictFromLocation = (
  ...
  // 1. Direct match with district names
  ...
  // 2. Exact or substring match in town dictionary
  // Sort by town name length descending so longer matching phrases match first (e.g. "Mount Lavinia" before "Lavinia")
...
export const getTownSuggestions = (input: string, limit = 5): TownLocation[] => {
  if (!input || input.trim().length < 2) return [];
  const q = input.toLowerCase().trim();
  return SRI_LANKA_TOWNS.filter(t => t.town.toLowerCase().includes(q)).slice(0, limit);
};
