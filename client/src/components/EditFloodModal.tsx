    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      ...
      >
        ...
        <div className="p-5 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-900">Update Flood Situation Report</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-xl bg-white border border-gray-300 text-slate-800 text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="relative">
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Location Name / Town <span className="text-red-600">*</span>
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2 rounded-xl bg-white border border-gray-300 text-slate-800 text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none"
                placeholder="e.g. Godagama, Wellampitiya, Biyagama, Galle..."
                required
              />
              {autoDetectedMatch && (
                ...
                <div className="absolute left-0 right-0 top-full mt-1 z-30 bg-white border border-gray-200 rounded-xl shadow-lg p-1.5 space-y-1">
                  {townSuggestions.map((item) => (
                    <button
                      ...
                      className="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-blue-50 text-xs flex items-center justify-between"
                    >
                      <span className="text-slate-800 font-semibold">📍 {item.town}</span>
                      <span className="text-[10px] text-slate-600 font-medium">{item.district}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Flood Category <span className="text-red-600">*</span>
              </label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Water Level (in feet) <span className="text-red-600">*</span>
              </label>
              <input
                type="number"
                step="0.1"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Situation Description & Update Notes <span className="text-red-600">*</span>
            </label>
            <textarea
              rows={3}
              value={formData.description || ''}
              onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              className="w-full px-3 py-2 rounded-xl bg-white border border-gray-300 text-slate-800 text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none leading-relaxed"
          ...
          <div className="pt-4 border-t border-gray-200 flex items-center justify-end gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
